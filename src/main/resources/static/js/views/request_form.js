...
function showRequestForm(id) {
    webix.ajax('doc_requests/' + id).then(function (data) {
        ...
        const isReadonly = data.statusReview == 0 ? false : true;
        requestFormBoby = getRequestFormBoby(data, isReadonly);
...
let getRequestFormMainBoby = (data, isReadonly) => {
    return {
        rows: [
...
let getRequestFormEmployee = (data, isReadonly) => {
    return {
        rows: [
...
let getRequestFormAddress = (data, isReadonly) => {
    return {
        rows: [
...
let getRequestFormObosn = (data, isReadonly) => {
    return {
        rows: [
...
let getRequestFormHistory = (data, isReadonly) => {
    return {
        rows: [
...
let getRequestFormBoby = (data, isReadonly) => {
    requestFormMainBoby = getRequestFormMainBoby(data, isReadonly);
    requestFormEmployee = getRequestFormEmployee(data, isReadonly);
    requestFormAddress = getRequestFormAddress(data, isReadonly);
    requestFormObosn = getRequestFormObosn(data, isReadonly);
    requestFormHistory = getRequestFormHistory(data, isReadonly);
    return {
        rows: [
            {
                id: 'form',
...
const minFunctionText = '№\tНаимнование органа власти\tОписание\n' +
