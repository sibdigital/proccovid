...
---------------------- Все, что выше, было взято из основного запроса
    filtered_tbl AS (
        SELECT *
        FROM main_tbl
        WHERE id_okved = :id_okved AND id_request_status = :id_status
    ) --
SELECT ft.id_okved as id_okved,
       cco.kind_code, cco.kind_name,
       ft.id_organization, ft.id_doc_request, ft.id_doc_request as id, ft.id_request_status, ft.time_send, ft.time_create,
